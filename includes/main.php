...
if (!function_exists('hkdigital_init_idbank_gateway_class')) {
    function hkdigital_init_idbank_gateway_class()
    {
        ...
        unload_textdomain('payment-gateway-for-idbank');
        load_textdomain('payment-gateway-for-idbank', $pluginBaseNameIdBank . '/languages/payment-gateway-for-idbank-hy.mo');
        load_textdomain('payment-gateway-for-idbank', $pluginBaseNameIdBank . '/languages/payment-gateway-for-idbank-en_US.mo');
        load_textdomain('payment-gateway-for-idbank', $pluginBaseNameIdBank . '/languages/payment-gateway-for-idbank-ru_RU.mo');
        load_plugin_textdomain('payment-gateway-for-idbank', false, $pluginBaseNameIdBank . '/languages');
        ...
        if (class_exists('WC_Payment_Gateway')) {
            class HKDigital_Idbank_Arca_Gateway extends WC_Payment_Gateway
            {
                ...
                public function __construct()
                {
                    ...
                    if (is_admin()) {
                        if (isset($_POST['hkd_idbank_checkout_id']) && sanitize_text_field($_POST['hkd_idbank_checkout_id']) != '') {
                            ...
                            $this->update_option('title', __('Pay via credit card', 'payment-gateway-for-idbank'));
                            $this->update_option('description', __('Purchase by credit card. Please, note that purchase is going to be made by Armenian drams. ', 'payment-gateway-for-idbank'));
                            $this->update_option('save_card_button_text', __('Add a credit card', 'payment-gateway-for-idbank'));
                            $this->update_option('save_card_header', __('Purchase safely by using your saved credit card', 'payment-gateway-for-idbank'));
                            $this->update_option('save_card_use_new_card', __('Use a new credit card', 'payment-gateway-for-idbank'));
                        }
                    }
                    ...
                    $this->save_card_button_text = !empty($this->get_option('save_card_button_text')) ? $this->get_option('save_card_button_text') : __('Add a credit card', 'payment-gateway-for-idbank');
                    $this->save_card_header = !empty($this->get_option('save_card_header')) ? $this->get_option('save_card_header') : __('Purchase safely by using your saved credit card', 'payment-gateway-for-idbank');
                    $this->save_card_use_new_card = !empty($this->get_option('save_card_use_new_card')) ? $this->get_option('save_card_use_new_card') : __('Use a new credit card', 'payment-gateway-for-idbank');
                ...
                public function CardsPageContent()
                {
                    ...
                    $html = '<div id="hkdigital_binding_info_idbank">';
                    ...
                    if (is_array($bindingInfo) && count($bindingInfo) > 0) {
                        ...
                        foreach ($bindingInfo as $key => $bindingItem) {
                            $html .= '<li class="card_item">
                                        ...
                                        ' . __($bindingItem['cardAuthInfo']['cardholderName'] . ' |  &#8226; &#8226; &#8226; &#8226; ' . $bindingItem['cardAuthInfo']['panEnd'] . ' (expires ' . $bindingItem['cardAuthInfo']['expiration'] . ')', 'payment-gateway-for-idbank') . '
                                         </span>
                                         <img src="' . $this->pluginDirUrl . 'assets/images/card_types/' . $bindingItem['cardAuthInfo']['type'] . '.png" class="card_logo big_img" alt="card"/>
                                         <svg  class="svg-trash-idbank" data-id="' . $bindingItem['bindingId'] . '" style="display: none" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                                            <path fill="#ed2353"
                    ...
                    if (is_user_logged_in() && $this->save_card && $this->binding_user_name != '' && $this->binding_password != '') {
                        ...
                        if (is_array($bindingInfo) && count($bindingInfo) > 0) {
                            ...
                            foreach ($bindingInfo as $key => $bindingItem) {
                                ...
                                $html .= '<svg  class="svg-trash-idbank" data-id="' . $bindingItem['bindingId'] . '" style="display: none" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                                            <path fill="#ed2353"
                                                  d="M32 464a48 48 0 0 0 48 48h288a48 48 0 0 0 48-48V128H32zm272-256a16 16 0 0 1 32 0v224a16 16 0 0 1-32 0zm-96 0a16 16 0 0 1 32 0v224a16 16 0 0 1-32 0zm-96 0a16 16 0 0 1 32 0v224a16 16 0 0 1-32 0zM432 32H312l-9.4-18.7A24 24 0 0 0 281.1 0H166.8a23.72 23.72 0 0 0-21.4 13.3L136 32H16A16 16 0 0 0 0 48v32a16 16 0 0 0 16 16h416a16 16 0 0 0 16-16V48a16 16 0 0 0-16-16z"></path>
                                        </svg>
                                    </li>';
                ...
                {
                    $debug = __('Log HKD ARCA Gateway events, inside <code>woocommerce/logs/idbank.txt</code>', 'payment-gateway-for-idbank');
                    if (!version_compare(WOOCOMMERCE_VERSION, '2.0', '<')) {
                        if (version_compare(WOOCOMMERCE_VERSION, '2.2.0', '<'))
                            $debug = str_replace('idbank', $this->id . '-' . date('Y-m-d') . '-' . sanitize_file_name(wp_hash($this->id)), $debug);
                        elseif (function_exists('wc_get_log_file_path')) {
                            $debug = str_replace('woocommerce/logs/idbank.txt', '<a href="/wp-admin/admin.php?page=wc-status&tab=logs&log_file=' . $this->id . '-' . date('Y-m-d') . '-' . sanitize_file_name(wp_hash($this->id)) . '-log" target="_blank">' . __('here', 'payment-gateway-for-idbank') . '</a>', $debug);
                        }
                    }
